...
from src.api.core.base_models import BaseConfig, RequestData, HttpStatus, DataType

T = TypeVar("T", bound=BaseModel)


class Builder:
    def __init__(self, session: httpx.Client, config: BaseConfig, headers: Optional[Dict[str, str]] = None):
    ...
    def parse_response(self, schema: Type[T]) -> Union[T, List[T], None]:
        if self.response_data is None:
            raise ValueError("No response data available. Please execute the request first.")
